...
| 切換 pane（方向鍵） | `M-S-Arrow` (Alt+Shift+Arrow) | 無需 prefix，方向鍵版本 |
| 切換 pane（prefix） | `<prefix> C-h/j/k/l` | 需要 prefix，可重複按 |
| 顯示 pane 編號 | `<prefix> q` | 顯示後快速按數字可跳轉 |

> `M-h/j/k/l` 和 `M-S-Arrow` 都會智慧判斷當前是 Neovim 還是 tmux，自動決定要移動 Neovim split 還是 tmux pane。
> `Alt+Arrow`（不帶 Shift）不攔截，保留給應用程式做 word jump。

### 調整 Pane 大小

| 操作 | 按鍵 | 說明 |
|------|------|------|
| 調整大小 | `<prefix> Alt+Arrow` | 每次 5 格（macOS 上 C-Arrow 被系統攔截） |
...
tmux 沒有把兩個 pane 的內容「合併」的功能，但有幾種常見需求對應的操作：

**關閉 pane，讓空間回收給旁邊的 pane**

```
<prefix> x   # 關閉目前 pane
```

**把其他 window 的 pane 拉進來（join-pane）**

```
<prefix> j               # 輸入來源 window 號碼，以水平方式合併進來
```

或用原生指令：

```
<prefix> :join-pane -s <window>.<pane>   # 從指定 window/pane 拉進來
<prefix> :join-pane -t <window>          # 把目前 pane 送到指定 window
```

> `-h` 水平（左右並排），`-v` 垂直（上下並排）。

**把 pane 獨立成新 window（break-pane）**

```
<prefix> !   # 把目前 pane 拆出去，變成一個新的 window
```

**切換左右 / 上下排列方向**

```
<prefix> Space   # 循環切換 5 種預設佈局（含水平、垂直）
```
...
左右寬度不對？ <prefix> Alt+Left/Right 調整
