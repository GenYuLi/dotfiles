...
| 操作 | 按鍵 | 說明 |
|------|------|------|
| 切換 pane（方向鍵） | `M-h/j/k/l` | 無需 prefix，Alt+vim 方向鍵，與 Neovim 共用 |
| 切換 pane（prefix） | `<prefix> C-h/j/k/l` | 需要 prefix，可重複按 |
| 顯示 pane 編號 | `<prefix> q` | 顯示後快速按數字可跳轉 |

> `M-h/j/k/l` 會智慧判斷當前是 Neovim 還是 tmux，自動決定要移動 Neovim split 還是 tmux pane。
...
<prefix> s  列表          <prefix> ,   重新命名      <prefix> z   全螢幕 toggle
<prefix> d  detach        <prefix> &   關閉          <prefix> Space 切換 layout
                                                   C-方向鍵     調整大小
```
